...
export function handleUseConsumable(heroId, itemId) {
	...
	if (type === 'heal_hp' && hero.hp.current < hero.hp.max) {
		...
		addToLog(`${hero.name} used ${itemData.name} and restored ${healedAmount} HP.`, hero.id); // MODIFIED
		used = true;
		...
		addToLog(`${hero.name} used ${itemData.name} and restored ${restoredAmount} MP.`, hero.id); // MODIFIED
		used = true;
	}
